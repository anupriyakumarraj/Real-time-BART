...
function getTransitInfo(){
        ...
        serverJSONRequest('/trips?source='+data.sourceStation +'&dest='+data.destStation,'GET',function(res){
                ...
                for(trip of trips){
                        ...
                        tripElement +=  '<div class="col-5"><h5>' + trip["@origTimeMin"] + '</h5><p>$' + trip["@fare"] + '
</p><p class="font-italic small">' + trip["@tripTime"] + ' mins</p></div>';
                        ...
                        if(first){//trip that will depart soonest
                                ...
                                if(departureTime.indexOf("PM") > 0){
                                        departureTime = (Number(departureTime.substring(0,2)) + 12) + departureTime.substr
ing(2,5);
                                }
                                else{
...
$(document).ready(function() {
        ...
        serverJSONRequest('/stations','GET',function(res){
                ...
                for(station of stations){
                        let stationListElement =  '<a class="list-group-item list-group-item-action" data-toggle="list" ab
br='+ station.abbr+' href> ' + station.name +'</a>';
